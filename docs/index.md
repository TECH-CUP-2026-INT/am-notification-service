...
Fútbol** university tournament. It is purely an **event consumer** from other
microservices and a **producer of in-app alerts** (bell-style) for the end
user: it never generates a notification on its own initiative, it always
reacts to something that happened in another service.
...
and exposes two equivalent entry points for events coming from other
services: **REST webhooks** and **RabbitMQ queues**, both invoking the same
domain ports (hexagonal architecture). Communication with the frontend is
REST-only, authenticated with the JWT already validated by the API Gateway.

### Goal

Give the end user a reliable, accessible notification bell, while fully
isolating the originating service from any failure or delay in processing:
...
| Spring AMQP + RabbitMQ | — | Second event entry point, with retries and DLQ |
| Spring Validation | — | Validation of inbound payloads |
| Spring Security | — | Gateway JWT (user) + internal API key (webhooks) |
...
| Testcontainers | — | Real MongoDB and RabbitMQ in integration tests |
| JaCoCo | 0.8.12 | Test coverage gate (≥ 80%) in CI |
| Docker + Docker Compose | — | Containerization and local orchestration (app + MongoDB + RabbitMQ) |

### Main features

- **Event intake** from other microservices (sanctions, chat messages,
  team requests/invitations, enrollment changes, match scheduling) through
  **9 REST webhooks** and their equivalent RabbitMQ queues.
...
- **Failure isolation**: the originating service is never blocked by this
  service (`202 Accepted` + asynchronous processing).
- **Automatic retries and dead-letter queue** for events arriving via
  RabbitMQ.
- **Publishing to Statistics** of a minimal event every time a notification
  is created.
- **Differentiated authentication** between service-to-service (internal
  API key) and end user (Gateway JWT), on the same Spring Security filter
  chain.
...
| [Architecture](arquitectura.md) | Hexagonal layers, data model, security and messaging (RabbitMQ) |
| [API](api.md) | REST endpoints, authentication and Swagger UI |
| [Service Integration](integracion-servicios.md) | Status of each inbound integration (confirmed or proposed) with the producing services |
| [Testing](pruebas.md) | Testing strategy and how to run it |
| [Team](equipo.md) | Members and roles of the TECH-CUP 2026 INT team |
| [Appendices](anexos.md) | Glossary, security findings and references |
...
```bash
# Starts MongoDB, RabbitMQ and the service (indexes and queues are created automatically)
