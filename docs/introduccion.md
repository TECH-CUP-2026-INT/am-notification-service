...
from other microservices (sanctions, chat messages, team requests/
invitations, enrollment changes, match scheduling), translates them into a
persisted notification, and exposes the history to the frontend.
...
- A failure or delay in this service never blocks the service that
  originated the event — webhooks respond `202 Accepted` and the
  notification is processed independently.
...
2. **The end user** (through the frontend), who queries their notification
   history authenticated with the JWT already validated by the API
   Gateway.
...
1. Receive card-sanction events from the Matches Service
   (`am-matches-service`, an integration owned by the astromerge team,
   **confirmed**).
2. Receive (once the owning teams confirm the contract) chat events,
   team requests/invitations, enrollment changes, and match scheduling
   events.
3. Translate each event into a persisted notification with an explicit
   `NotificationType` and a readable message.
4. Expose the authenticated user's notification history, unread count, and
...
|---|---|
| Deciding *when* a notifiable event occurs | The originating service (Matches, Communications, Teams, Enrollment, Scheduling) |
| Sending real push/email notifications outside the app | Out of scope for the current platform |
| Authenticating and validating the signature of the user's JWT | API Gateway |

See [Architecture](arquitectura.md) for the detail of which integrations
are confirmed and which are still pending from other teams.
