...
| RF-07 | ~~Notificación de inscripción completada con comprobante, al Organizador~~ | 🚫 Dado de baja — con la integración de MercadoPago el pago se confirma directamente y ya no existe un comprobante que subir; Inscripción confirmó que el flujo pasa por RF-06 (`INSCRIPCION_APROBADA`) en su lugar. Endpoint `POST /api/notificaciones/inscripciones/comprobante` removido. |
