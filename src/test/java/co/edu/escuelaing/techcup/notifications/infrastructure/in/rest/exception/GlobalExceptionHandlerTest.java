...
class GlobalExceptionHandlerTest {
    ...
    void handleNotFound_buildsNotFoundResponse() {
        ...
        assertBody(response.getBody(), HttpStatus.NOT_FOUND, "No encontrado", "No existe la notificación " + id);
    }
    ...
    void handleNotificationAccessDenied_buildsForbiddenResponse() {
        ...
        assertBody(response.getBody(), HttpStatus.FORBIDDEN, "Acceso denegado", "No tiene permisos sobre la notificación " + id);
    }
    ...
    void handleUnreadableBody_buildsBadRequestWithFixedMessage() {
        ...
        assertBody(response.getBody(), HttpStatus.BAD_REQUEST, "Solicitud inválida", "Cuerpo de la solicitud inválido o incompleto");
    }
    ...
    void handleUnauthenticated_buildsUnauthorizedResponse() {
        ...
        assertBody(response.getBody(), HttpStatus.UNAUTHORIZED, "No autenticado", "no auth");
    }
    ...
    void handleForbidden_buildsForbiddenWithFixedMessage() {
        ...
        assertBody(response.getBody(), HttpStatus.FORBIDDEN, "Acceso denegado", "No tiene permisos suficientes para esta acción");
    }
    ...
    void handleUnexpected_buildsInternalServerErrorWithFixedMessage() {
        ...
        assertBody(response.getBody(), HttpStatus.INTERNAL_SERVER_ERROR, "Error interno", "Ocurrió un error inesperado");
    }

    private void assertBody(ErrorResponse body, HttpStatus status, String errorEnEspanol, String message) {
        assertThat(body.status()).isEqualTo(status.value());
        assertThat(body.error()).isEqualTo(errorEnEspanol);
